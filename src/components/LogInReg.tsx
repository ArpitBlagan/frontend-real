...
const LogInReg = () => {
  ...
  const handleRegister=async(event:React.MouseEvent<HTMLButtonElement>)=>{
    ...
    if(email==""||password==""){
      ...
      const res=await axios.post("https://real-backend-5tk8.onrender.com/register",body,{withCredentials:true});
      console.log(res);
  ...
  const handleLogin=async(event:React.MouseEvent<HTMLButtonElement>)=>{
    ...
    const body={email,password};
    const res=await axios.post("https://real-backend-5tk8.onrender.com/login",body,{withCredentials:true});
    if(res.data.message=="Logged IN"){ 
      if(value){
        value.setLoggedIn(true);
      }
